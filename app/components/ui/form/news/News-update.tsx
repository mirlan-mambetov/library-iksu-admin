...
export const NewsUpdate: FC<{ id: number }> = ({ id }) => {
	const { data: newsData, isLoading: newsLoading } =
		newsApi.useFetchNewsByIdQuery(id, { skip: !id })
	const { handlerClose } = useModalComponent()
	const toastr = useToast()
	const [file, setFile] = useState(null)
	const [updateNews, { isLoading }] = newsApi.useUpdateNewsMutation()
	...
	const submitHandler = async (values, actions) => {
		try {
			...
			await updateNews({ id, data: formData })
				.unwrap()
				.then(() => handlerClose())
		} catch (err) {
