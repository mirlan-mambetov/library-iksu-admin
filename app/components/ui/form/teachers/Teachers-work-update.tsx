...
export const TeachersWorkUpdate: FC<{ id: number }> = ({ id }) => {
	const { data: work, isLoading: loadingData } =
		teachersApi.useFetchOneWorkQuery(id, { skip: !id })
	const toastr = useToast()
	const { handlerClose } = useModalComponent()
	const [file, setFile] = useState(null)
	const [updateWork, { isLoading }] =
		teachersApi.useUpdateTeachersWorkMutation()
	...
	const submitHandler = async (values, actions) => {
		try {
			...
			await updateWork({ id, body: formData })
				.unwrap()
				.then(() => handlerClose())
		} catch (err) {
