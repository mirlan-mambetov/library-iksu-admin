...
import { IVestnikMaterial } from '@/interfaces/Vestnik-interface'
import {
	Button,
	FormControl,
	FormErrorMessage,
...
export const UpdateVestnikMaterial: FC<{ id: number }> = ({ id }) => {
	const [file, setFile] = useState(null)
	const [materialId, setMaterialId] = useState(null)
	const toastr = useToast()
	const { handlerClose } = useContext(ModalComponentInitialContext)
	const [updateMaterial] = vestnikApi.useUpdateMaterialMutation()
	const { data: oldData, isLoading } = vestnikApi.useFetchMaterialByIdQuery(
		id,
		{ skip: !id }
	)
	...
	const submitHandler = async (values, actions) => {
		try {
			...
			await updateMaterial({ id, data: formData })
				.unwrap()
				.then(() => handlerClose())
		} catch (err) {
