...
export const UpdateTabsLink: FC<{ id: number }> = ({ id }) => {
	const { handlerClose } = useContext(ModalComponentInitialContext)
	const [file, setFile] = useState(null)
	const toastr = useToast()
	const [updateTabLink, { isLoading, isError }] =
		tabsApi.useUpdateTabLinkByIdMutation()
	...
	const submitHandler = async (values, actions) => {
		try {
			...
			await updateTabLink({ id, data: formData })
				.unwrap()
				.then(() => handlerClose())
		} catch (err) {
